        ...
        >
          {isLoading ? (
            ...
            results.map((ex) => (
              <div
                key={ex.id}
                onMouseDown={(e) => { e.preventDefault(); onSelect(ex); setQuery(""); setOpen(false); }}
                style={{
                  padding: "12px",
                  cursor: "pointer",
    ...
    return (
      <PageShell title="Edit Workout" subtitle="Customize exercises & sets">
        <div style={{ display: "flex", flexDirection: "column", gap: "20px", paddingBottom: "80px" }}>
              ...
              {ex.sets.map((set, si) => (
                <div key={si} style={{ display: "flex", gap: "8px", marginBottom: "8px", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <input type="number" value={set.weight} onChange={(e) => updateSet(ei, si, "weight", e.target.value)} style={{ width: "60px", padding: "6px 8px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.5)", color: "#fff", outline: "none", textAlign: "right", fontFamily: "var(--font-display)", fontWeight: 700 }} />
                    <span style={{ fontSize: "10px", color: "var(--text-tertiary)", fontWeight: 600 }}>lbs</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <input type="number" value={set.reps} onChange={(e) => updateSet(ei, si, "reps", e.target.value)} style={{ width: "50px", padding: "6px 8px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.5)", color: "#fff", outline: "none", textAlign: "right", fontFamily: "var(--font-display)", fontWeight: 700 }} />
                    <span style={{ fontSize: "10px", color: "var(--text-tertiary)", fontWeight: 600 }}>reps</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    <input type="number" value={set.rir !== undefined ? set.rir : 0} onChange={(e) => updateSet(ei, si, "rir", e.target.value)} style={{ width: "40px", padding: "6px 8px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.1)", background: "rgba(0,0,0,0.5)", color: "#fff", outline: "none", textAlign: "right", fontFamily: "var(--font-display)", fontWeight: 700 }} />
          ...
          <button onClick={() => setIsEditing(false)} style={{ position: "fixed", bottom: "80px", left: "20px", width: "calc(100% - 40px)", padding: "16px", borderRadius: "16px", background: "#30D158", color: "#000", fontFamily: "var(--font-display)", fontSize: "16px", fontWeight: 800, border: "none", cursor: "pointer", zIndex: 100 }}>
            Done Editing
          </button>
        </div>
      </PageShell>
    ...
    return (
      <PageShell title="Push Day" subtitle="Chest · Triceps · Shoulders">
        ...
        <div style={{ padding: "8px", borderRadius: "16px", border: "1px solid rgba(255,255,255,0.12)", background: "rgba(0,0,0,0.1)", marginBottom: "16px" }}>
          ...
          {workoutPlan.map((ex, ei) => (
            <div key={ex.id || ei} className={`glass-card animate-fade-up delay-${Math.min(ei + 2, 6)}`} style={{ padding: "14px 16px", marginBottom: "8px", display: "flex", alignItems: "center", gap: "12px" }}>
